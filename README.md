...
```
content/
  home.ts             — `homeCases`: фон → `layerPhoto` → HTML; **`caseHeroTitleStyle` / `caseHeroTagsLineStyle`** — общая типографика всех трёх hero (заголовок `#FFFFFF99` + `case-hero-title-spec`; теги `·`, кроме **auth** — только `tagsColor` золото); метрики — `case-hero-stats-inner` везде одинаково
app/
  ...
  globals.css                  — глобальные стили; на кейсе мобилка: `section.px-5` без padding; у **promo-ab** свой `object-position` для `.case-photo-native` (отдельный мокап браузера)
  case/auth-redesign/page.tsx  — кейс: редизайн флоу (моб. h1 — одна строка + ellipsis; desktop hero может быть `hero-title.png` — пересобери экспорт под новый текст)
components/
  ...
  CaseStudy.tsx       — `case-hero-card`: метрики снизу (`case-hero-stats-inner`) на desktop **24px / medium / 125%**, цвет **#FFFFFF9E**; заголовок с `copyTitleStyle` — `case-hero-title-spec` + `globals.css` (**24px / 500**)
...
public/
  ...
  images/cases/       — ассеты из Figma **Maket/b** [`node-id=2693-159496`](https://www.figma.com/design/vCzZlNer2Fixkg9ibPRxR0/?node-id=2693-159496): герои `figma-case-auth-hero.png`, `figma-case-verification-hero.png`; **промо** — два слоя: `case-promo-bg.png` (волны) + `case-promo-browser.png` (мокап браузера), в коде `layerBackgroundImage` + `layerPhoto`; старый композит `figma-case-promo-layer.png` можно не использовать. сабкарты `figma-auth-sub*.png`, `figma-verification-sub*.png` (MCP в репо; URL в Figma протухают — переэкспорт)
...
- макет из figma, 1440px; canvas **#0a0a0a** (Neutral/1000), карточки кейсов в основном **#191919**, порядок **`homeCases`**: **auth** (два телефона, сплит-фон) → **promo-ab** → **verification** (рука); заливки **`#262626`** у auth и verification, чипы опыта **#323232**, вторичный текст ссылок **#494a4d**
